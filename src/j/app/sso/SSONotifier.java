...
public class SSONotifier implements Runnable{
	 ...
	 * @param type
	 */
	public static void addTask(Client client,String globalSessionId,String userId,String userIp,String type){
		SSONotifier notifier=getNotifier(client);
		notifier.tasks.add(new Object[]{client,globalSessionId,userId,userIp,type});
	}
	...
	public void run(){
		while(true){
			...
			for(int i=0;i<tasks.size();i++){
				...
				String userIp=(String)cells[3];
				String type=(String)cells[4];
				
				if(type.equals(type_login)){
					this.login(client,globalSessionId,userId,userIp);
				}else if(type.equals(type_logout)){
					this.logout(client,globalSessionId,userId,userIp);
				}else{
					this.logout(client);
				}
				i--;
