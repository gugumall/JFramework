...
public void setQrcodeVersion(int ver){
     ...
     *���󥳡��ɥ⡼�ɤ����ꤷ�ޤ���
     *'N':�����⡼��  'A':�ѿ����⡼�ɡ�����¾:8bit byte�⡼��
     *@param encMode ���󥳡��ɥ⡼��('N','A' or other)
     *
     */
...
public boolean[][] calQrcode(byte[] qrcodeData){
	...
	for (int i=0;i<dataLength;i++){
		...
		if (chr>=65 && chr<91){
		    ...
		    if (chr==32){chrValue=36;}
                    if (chr==36){chrValue=37;}
                    if (chr==37){chrValue=38;}
                    if (chr==42){chrValue=39;}
                    if (chr==43){chrValue=40;}
                    if (chr==45){chrValue=41;}
                    if (chr==46){chrValue=42;}
                    if (chr==47){chrValue=43;}
                    if (chr==58){chrValue=44;}
		}
	    }
            if ((i % 2)==0){
